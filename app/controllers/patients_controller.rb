class PatientsController < ApplicationController
  ...
  def show
    ...
    @medications = @patient.medications.map do |medication_order|
      [medication_order.name, ['%g' % ('%.2f' % medication_order.dosage), medication_order.unit].join, medication_order.route.upcase, [medication_order.order_frequency.value, medication_order.order_frequency.unit].join].join(' ')
    end.join(', ')
  end
